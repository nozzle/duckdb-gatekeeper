...
// Authorizes one catalog entry the binder retrieved against each layer, ceiling first: tables and views by
// identity against table policy, function entries against the never-bind list and, when the name is attributable
// to the caller, the layer's blocks. Records the violation and throws PermissionException at the first denial.
...
// plan: the never-bind list everywhere, blocks where provenance attributes the name to the caller. One walk of
// the plan per layer, the ceiling's first, so a plan both layers deny is reported by the ceiling's walk.
