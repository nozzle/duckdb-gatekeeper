...
struct TextCheck {
	...
	struct Unit {
		...
		// Enum types the statements before it in the same batch create, which this statement names in PIVOT
		// IN lists. Nonempty only for the final SELECT of a dynamic PIVOT checked as a whole, where those
		// types do not exist yet: Authorize binds it against placeholder values instead (see PivotShapes).
