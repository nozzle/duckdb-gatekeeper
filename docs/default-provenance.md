...
The reproducible checks are each collection's `qualified-report.json`, generated by
`scripts/audit_inventory.py --collection DIR` from hash-verified reconstructed stages
and the current source map. `--collection DIR --extension NAME` audits one full final
target; collector `base.json` also works directly with `--candidate`. See the
[offline audit interface](../inventories/README.md#offline-collection-audit-and-reconstruction).
The qualified reports enumerate observed and missing default identities, ungranted
default-name identities, unknown names, and per-target source-kind coverage. Verification
compares complete sets and policy hashes, so substituting one missing name or changing
Excel's scalar kind to table cannot pass merely by preserving a count.

`test/test_collection_audit.py` runs in normal pytest CI against both collections,
without native extension replays. It explicitly checks the version-specific missing
sets above, reporting-only window labels, optional-extension registration kinds,
dependency/target stages, and corruption of base/stage/final-delta/lock hashes.
Unknown additions are reported and never granted. The historical `report.json` files
retain exploratory fields as evidence; their aggregate coverage claims are recomputed
too, while current per-stage policy checks use the qualified reports. No historical
