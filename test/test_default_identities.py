...
from audit_inventory import capture_functions, compare, qualified_drift
...
def test_default_mapping_accounts_for_every_historical_name():
    ...
    assert len(identities) == 932 and len(granted) == 913 and len(excluded) == 40
    ...
    assert kinds("row_number") == {"aggregate", "window"}
...
def test_identity_kinds_cross_check_historical_snapshot_without_promoting_it():
    ...
    for identity in load_default_identities():
        ...
        if kind == "window" or (kind == "scalar" and name in {"unnest", "round_even", "roundbankers"}):
