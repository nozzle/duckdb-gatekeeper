...
def test_fallback_requires_both_layers_and_records_fixed_identity(db):
    ...
    assert not validate(db, "SELECT $x", {"allowed_functions": ["getvariable"]})["allowed"]
    configure(db, {"use_default_functions": False, "allowed_functions": ["getvariable"]})
    ...
    configure(db, {"allowed_functions": ["getvariable"], "blocked_functions": ["getvariable"]})
...
def test_trusted_view_is_not_a_caller_reference(db):
    ...
    configure(db, {"allowed_functions": ["m"], "blocked_functions": ["getvariable"]})
