...
FROM gatekeeper_validate('SELECT * FROM unnested', blocked_functions := ['unnest']);

SELECT CASE WHEN NOT coalesce(code = 'forbidden', false)
            THEN error('block did not reach the caller''s unnest: ' || code) END
FROM gatekeeper_validate('SELECT unnest([1, 2])', blocked_functions := ['unnest']);
...
SELECT CASE WHEN NOT coalesce(current_setting('gatekeeper_policy').blocked_functions = ['md5'], false)
