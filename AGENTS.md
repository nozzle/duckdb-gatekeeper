...
- Python requirements are edited in `requirements*.in` and locked with hashes into `requirements*.txt` via `pip-compile --generate-hashes --strip-extras --allow-unsafe`; regenerate the lock files whenever an `.in` file changes.
